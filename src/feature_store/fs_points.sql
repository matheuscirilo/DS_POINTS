    ...
    SELECT idCustomer,
        ...
        SUM(
            CASE
                ...
                ELSE 0
            END
        ) AS pointsAcumuladosD21,
        SUM(
            CASE
                WHEN pointsTransaction > 0
                AND dtTransaction >= DATE('{date}', '-14 day') THEN pointsTransaction
                ELSE 0
            END
        ) AS pointsAcumuladosD14,
        SUM(
            CASE
                WHEN pointsTransaction > 0
                AND dtTransaction >= DATE('{date}', '-7 day') THEN pointsTransaction
                ELSE 0
            END
        ) AS pointsAcumuladosD7,
        SUM(
            CASE
                WHEN pointsTransaction < 0 THEN pointsTransaction
                ELSE 0
            END
        ) AS pointsResgatadosD21,
        SUM(
            CASE
                WHEN pointsTransaction < 0
                AND dtTransaction >= DATE('{date}', '-14 day') THEN pointsTransaction
    ...
    SELECT t1.idCustomer,
        ...
        SUM(
            CASE
                ...
                ELSE 0
            END
        ) AS pointsAcumuladosVida,
        SUM(
            CASE
                WHEN t2.pointsTransaction < 0 THEN t2.pointsTransaction
                ELSE 0
            END
        ) AS pointsResgatadosVida,
        CAST(
            MAX(julianday('{date}') - julianday(dtTransaction)) AS INTEGER
        ) + 1 AS diasVidas
...
tb_join AS (
    SELECT t1.*,
        ...
        1.0 * t2.pointsAcumuladosVida / t2.diasVidas AS pointsPorDia
    FROM tb_pontos_d AS t1
